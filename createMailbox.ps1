...
foreach($user in $newusers) {
         ...
         -or $userMemberships.MemberOf -contains "CN=DELG-G-O365Kiosk,OU=Groups,OU=Centerlight,DC=Centerlight,DC=CL,DC=LOCAL") 
   {
        if($user.EmailAddress -notmatch "@kinshiphs.com") 
        {
            ...
            Set-ADUser -Identity $user.samaccountname -remove @{proxyAddresses="SMTP:$centerlightEmail"} 
            Set-ADUser -Identity $user.samaccountname -Add @{proxyAddresses="SMTP:$kinshipEmail"}
            Set-ADUser -Identity $user.SamAccountName -EmailAddress $kinshipEmail
