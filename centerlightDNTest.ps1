...
$DomainName = "CLMServices"
#DomainName = "C2QHealthSolutions"
#DomainName = "TeamCareMed"
#DomainName = "MatterOfCare"


$password = "xrD~da3=47vOk5Hd=pZ^"
$securePassword =  ConvertTo-SecureString $password -AsPlainText -Force

$LogFolder = "D:\ExchangeMailboxCreation\Logs_$DomainName"
...
function generateEmailAlias($samAccountName) 
{
    ...
    while($unique -eq $false) 
    {
        ...
        if($allemails -notcontains $($temp + "@$DomainName.com")) 
        {
            $emailAlias = $temp
            $unique = $true
        } 
...
$newusers = Get-ADUser -Filter * -SearchBase "OU=users,OU=$DomainName,DC=Centerlight,DC=cl,DC=local" -server 001100SADC01  -Properties "cn", "EmailAddress", "whenCreated", "EmployeeID", "mail" | Where-Object {$_.whenCreated -ge (Get-Date).AddDays(-1)} 
...
{ 
    ...
    if($user.UserPrincipalName -notmatch "@$DomainName $emaildotc") 
    {
        ...
        $DomainNameEmail = $emailalias + "@$DomainName.com" #set $DomainName email as a variable
        $employeeEmail = $employeeID + "@$DomainName.com"#create email using employeeID
        $onMicrosoftEmail = "smtp:" + $emailAlias + "@centerlight.mail.onmicrosoft.com" #create onMicrosoft email
        $emailProxy = "SMTP:" + $DomainNameEmail
        ...
        else
        {
            ...
            Set-ADUser -Server 001100SADC01 $employeeID -Replace @{UserPrincipalName=$employeeEmail} #set user principalname to $DomainName email
            Set-ADUser -Server 001100SADC01 $employeeID -Replace @{mail=$DomainNameEmail} #set user mail field to $DomainName email
            Set-ADUser -Server 001100SADC01 $employeeID -emailaddress $DomainNameEmail #set user mail field to $DomainName email 
       ...
       {   
            ...
            enable-remotemailbox $user.Name -remoteroutingaddress "$emailAlias@centerlight.mail.onmicrosoft.com" -PrimarySmtpAddress $DomainNameEmail -Alias $emailAlias -DomainController 001100SADC01
